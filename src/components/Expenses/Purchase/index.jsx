...
const Purchase = ({purchases, purchase, setPurchases, setIsOpen}) => {
    ...
    return(
        <div className='flex items-center py-3 border-b-2 border-b-gray-300' onClick={(e) => e.target && setIsOpen(false)}>
            <div className='flex justify-between items-center w-full pr-5'>
                ...
                <DeleteModal deletePurchase={deletePurchase} isDelete={isDelete} setIsDelete={setIsDelete} purchase={purchase}/>
            ...
            <div className={ isDelete ? "fixed top-0 right-0 left-0 bottom-0 bg-gray-600 opacity-30 z-20" : "hidden"} onClick={(event) => event.target && setIsDelete(false)}></div>
        </div>
    )
}
