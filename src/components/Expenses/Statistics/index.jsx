...
const Statistics = ({filteredPurchase, options}) => {
    ...
    const COLORS = ['#7dd3fc', '#fed7aa', '#86efac', '#d1d5db', '#c4b5fd', '#fef08a', '#f9a8d4']
    const colorsForClassName = ['bg-sky-300', 'bg-orange-200', 'bg-green-300', 'bg-gray-300', 'bg-violet-300', 'bg-yellow-200', 'bg-pink-300']
    ...
    const renderCustomizedLabel = ({ cx, cy, midAngle, innerRadius, outerRadius, percent, index }) => {
        ...
        return (
          <text x={x} y={y} fill="white" textAnchor={x > cx ? 'start' : 'end'} dominantBaseline="central" className='opacity-0'>
            ...
            <div className='flex justify-between items-center statistic'>
                ...
                <div className="pt-8 flex flex-col">
                    ...
                    {options.map((option, index) => {
                        return(
                            <li className={colorsForClassName[index] + ' rounded-md px-5 font-medium'} key={option}>{option} - {filterCategories(index).toLocaleString('ru-RU')}.00 ₽</li>
                        )
                    })}
                    </ul>
                    <p className='font-medium text-center text-lg'>Итого - {allCategories()}</p>
                </div>
            </div>
        </div>
