...
const HomeStatistic = ({ copyPurchases, copyIncome, budget, budgetValue }: Props) => {
    let budgetValueArr = budget.length > 0 ? budgetValue.split('.') : getAmount(copyIncome).split('.')
    ...
    // @ts-ignore
    let budgetInput = budget.length > 0 ? Number(budgetValueArr[0].match(/\S/g).join("")) : Number(getAmount(copyIncome).split('.')[0].match(/\S/g).join(""))
